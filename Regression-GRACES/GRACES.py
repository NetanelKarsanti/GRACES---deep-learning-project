...
class GraphConvNet(nn.Module):
    def __init__(self, input_size,  hidden_size, alpha):
        ...
        self.output = nn.Linear(hidden_size[-1], self.output_size)
...
class GRACES:
    def __init__(self, n_features, hidden_size=None, q=2, n_dropouts=10, dropout_prob=0.5, batch_size=40,
                 learning_rate=0.001, epochs=50, alpha=0.95, sigma=0.1, f_correct=0): # chnge batch size from 16 to 40 & sigma frome 0 to 0.1
    ...
    def f_test(self, x, y):
        slc = SelectKBest(mutual_info_regression , k=x.shape[1])
        slc.fit(x, y)
    ...
    def train(self, x, y):
        ...
        self.model = GraphConvNet(input_size,  self.hidden_size, self.alpha)
    ...
    def gradient(self, x, y, model):
        model_gr = GraphConvNet(x.shape[1],  self.hidden_size, self.alpha)
    ...
    def select(self, x, y, score=None):
        x = torch.tensor(x)
        ...
        while len(self.S) < self.n_features + 1:
            self.train(x, y[:,0])
            input_gradient = self.average(x, y[:,0], self.n_dropouts)
