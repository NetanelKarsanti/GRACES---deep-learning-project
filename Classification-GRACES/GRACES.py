...
class GRACES:
    def __init__(self, n_features, hidden_size=None, q=2, n_dropouts=10, dropout_prob=0.5, batch_size=16,
                 learning_rate=0.001, epochs=50, alpha=0.95, sigma=0.1, f_correct=0):
    ...
    def train(self, x, y):
        ...
        x = x[:, self.S]
        optimizer = torch.optim.Adam(self.model.parameters(), lr=self.learning_rate) # שימוש לעדכון הפרמטרים של הרשת
        train_set = []
        for i in range(x.shape[0]):
            train_set.append([x[i, :], y[i]])
        train_loader = torch.utils.data.DataLoader(train_set, batch_size=self.batch_size, shuffle=True) # כאן לדעתי צריך להשאיר את ערבוב הנתונים
        for e in range(self.epochs):
            for data, label in train_loader:
                input_0 = data.view(data.shape[0], -1)
                optimizer.zero_grad()
                output = self.model(input_0.float())
                loss = self.loss_fn(output, label.long()-6)
                loss.backward()
                optimizer.step()
    ...
    def gradient(self, x, y, model):
        ...
        loss_gr = self.loss_fn(output_gr, y.long()-6)
        loss_gr.backward()
